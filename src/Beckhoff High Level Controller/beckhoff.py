...
# Try to import pyads, but allow running without it (for simulation mode)
try:
    import pyads
except ImportError:
    pyads = None
...
# ==============================
# Fake PLC for Simulation Mode
# ==============================
class FakePLC:
    """Simple fake PLC that mimics pyads.Connection interface."""
    ...
    def write_by_name(self, name, value, plctype=None):
        self._vars[name] = value

    def read_by_name(self, name, plctype=None):
        if name in self._vars:
            return self._vars[name]
        if plctype is not None and "BOOL" in str(plctype):
            return False
        return 0.0


# ==============================
# Beckhoff Control Logic
# ==============================
class BeckhoffSystem:
    ...
    def connect(self, ams_net_id, port=851, simulation=False):
        self.disconnect()
        ...
        try:
            if simulation or pyads is None:
                self.plc = FakePLC()
                self.plc.open()
                self.plc.read_state()
                self.connected = True
                return True, "Simulation mode (Fake PLC) connected."
            else:
                self.plc = pyads.Connection(ams_net_id, port)
                self.plc.open()
                self.plc.read_state()
                self.connected = True
                return True, "Connected to real PLC."
    ...
    def disconnect(self):
        try:
            if self.plc and getattr(self.plc, "is_open", False):
                self.plc.close()
        except Exception:
            pass
        self.connected = False
    ...
    def write_bool(self, var_name, value):
        if not self.connected or self.plc is None:
            return
        try:
            if not self.simulation and pyads is not None:
                self.plc.write_by_name(var_name, value, pyads.PLCTYPE_BOOL)
            else:
                self.plc.write_by_name(var_name, value)
        except Exception as e:
            self._mark_error(e)
    ...
    def write_float(self, var_name, value):
        ...
        try:
            v = float(value)
            if not self.simulation and pyads is not None:
                self.plc.write_by_name(var_name, v, pyads.PLCTYPE_LREAL)
            else:
                self.plc.write_by_name(var_name, v)
        except Exception as e:
            self._mark_error(e)
    ...
    def read_bool(self, var_name):
        ...
        try:
            ...
            return bool(self.plc.read_by_name(var_name))
        except Exception as e:
            self._mark_error(e)
            return False
...
class BeckhoffApp:
    def __init__(self, root):
        ...
        # Polling
        self.root.after(200, self.poll_sensor)
    ...
    def _init_ui(self):
        ...
        self.ams_entry.insert(0, "127.0.0.1.1.1")
        ...
        ttk.Checkbutton(conn_frame, text="Simulation", variable=self.sim_var).pack(side="left", padx=5)
        ...
        ttk.Scale(
            conv_frame, from_=0, to=100, orient="horizontal",
    ...
    def toggle_connection(self):
        if not self.system.connected:
            self.start_connect(simulation=self.sim_var.get(), auto=False)
        else:
    ...
    def start_connect(self, simulation=False, auto=False):
        ...
        def worker():
            success, msg = self.system.connect(ams, port=port, simulation=simulation)
            self.root.after(0, lambda: self.finish_connect(success, msg, simulation, auto))

        self.connect_thread = threading.Thread(target=worker, daemon=True)
        self.connect_thread.start()

    def finish_connect(self, success, msg, simulation, auto):
        if success:
            self.lbl_conn_status.config(
                text="Connected (Sim)" if simulation else "Connected",
                foreground="green"
            )
            self.set_led("green")
    ...
    def reconnect_if_needed(self):
        if (
            ...
            and not self.sim_var.get()
            and self.auto_reconnect_var.get()
        ):
            now = time.time()
            if now - self.last_reconnect_attempt > 5:
                self.last_reconnect_attempt = now
                self.log("Auto reconnecting...")
                self.start_connect(simulation=False, auto=True)
    ...
    def toggle_conveyor(self):
        ...
        if self.conveyor_state:
            self.btn_conveyor.config(text="Stop Conveyor")
            self.update_speed(self.speed_var.get())
            self.log("Conveyor started.")
        else:
            self.btn_conveyor.config(text="Start Conveyor")
    ...
    def toggle_gripper(self):
        ...
        if self.gripper_state:
            self.btn_gripper.config(text="Release Suction")
            self.log("Suction activated.")
        else:
            self.btn_gripper.config(text="Activate Suction")
            self.log("Suction released.")
    ...
    def poll_sensor(self):
        ...
        if self.system.connected:
            sensor_val = self.system.read_bool("MAIN.bProductDetected")

        self.canvas_sensor.itemconfig(
            self.sensor_light,
            fill="#00FF00" if (self.system.connected and sensor_val) else "gray"
        )
        self.lbl_sensor_text.config(
            text="PRODUCT DETECTED" if sensor_val else "No Product"
        )

        if (
            self.system.connected
            and self.auto_mode_var.get()
            and sensor_val
            and not self.auto_sequence_running
        ):
        ...
        if now - self.last_log_time > 1:
            ...
            self.data_samples.append((now, speed, int(self.conveyor_state), int(sensor_val)))
